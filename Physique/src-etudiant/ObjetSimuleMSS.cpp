...
void ObjetSimuleMSS::initMeshObjet()
{
    ...
    /*    m_ObjetSimule = Mesh(GL_TRIANGLES);
    for (uint ii = 0; ii < _VISize; ii++)
    {
        // Texture
        Texture tex = _vectTexture[_VIndices[ii]];
        vec2 v2(tex.u, tex.v);
        m_ObjetSimule.texcoord(v2);

        // Normale
        Vector v = _vectNormals[_VIndices[ii]];
        vec3 v3(v);
        m_ObjetSimule.normal(v3);
    ...
    // Faces
    for (uint ii = 0; ii < _NFacets; ii++) {
        m_ObjetSimule.triangle(_VIndices[ii*3], _VIndices[(ii*3)+1], _VIndices[(ii*3)+2]);
    }*/
}
...
void ObjetSimuleMSS::updateVertex()
{
    ...
    for (uint ii = 0; ii < P.size(); ++ii)
    {
        ...
        Vector v = _vectNormals[ii];
        vec3 v3(v);
        m_ObjetSimule.normal(ii, v3);

        // Vertex
        v = P[ii];
        v3 = vec3(v);
        m_ObjetSimule.vertex(ii, v3);
    }
}
...
void ObjetSimuleMSS::Simulation(Vector gravite, float viscosite, int Tps)
{
    ...
    // Affichage des positions
     if(Tps == 0) AffichagePos(Tps);

    /** Modification des normales **/
    setNormals();


    if (key_state(SDLK_k)) { lacheTissu(); }
}


void ObjetSimuleMSS::lacheTissu() {
    for(unsigned int ii = 0; ii < M.size(); ii++) {
        if(M[ii] == 0.0) M[ii] = 1.f;
    }
}